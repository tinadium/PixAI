...
DB_FILE = "users_db.json"  # Fichier JSON de sauvegarde persistante
...
PROFILS_IA = {
    ...
    "💻Nova1.6-codex": {
        "description": (
            "Optimisé pour la programmation, la revue de code et le debug."
        ),
        "temperature": 0.2,
        "system_instruction": """
...
# ==============================================================================
# 💾 GESTION PERSISTANTE DE LA BASE DE DONNÉES JSON
# ==============================================================================
def load_db():
...
def update_user_credits(user_email, new_credits):
  db = load_db()
  if user_email in db:
    db[user_email]["credits"] = max(0, new_credits)
    save_db(db)
...
if not st.session_state.user_email:
  ...
  st.markdown(
      '<div class="sub-title">Entrez un identifiant pour accéder à l\'assistant'
      " et vos crédits quotidiens.</div>",
...
# ==============================================================================
# 🪙 INITIALISATION & GESTION PERSISTANTE DES CRÉDITS
# ==============================================================================
...
# Si c'est un nouvel utilisateur ou première connexion du jour
if user_email not in db:
  db[user_email] = {"credits": CREDITS_QUOTIDIENS, "last_login_date": today_str}
  save_db(db)
  st.toast(
      f"🎁 Vos {CREDITS_QUOTIDIENS} crédits quotidiens sont prêts !", icon="🪙"
  )
elif db[user_email].get("last_login_date") != today_str:
  # Réinitialisation quotidienne des crédits à la date du jour
  db[user_email]["credits"] = CREDITS_QUOTIDIENS
  db[user_email]["last_login_date"] = today_str
  save_db(db)
  st.toast(
      f"🎁 Vos {CREDITS_QUOTIDIENS} crédits quotidiens ont été rechargés !",
      icon="🪙",
  )

# Synchronisation dans la session Streamlit
st.session_state.credits = db[user_email]["credits"]
...
def get_client():
  ...
  if not key:
    st.error(
        "🔑 Clé API introuvable. Indique ta clé dans la variable `API_KEY` en"
        " haut du script."
    )
    st.stop()
...
with st.sidebar:
  ...
  profil_choisi = st.selectbox(
      "Choisis le modèle:", options=list(PROFILS_IA.keys()), index=0
  )
...
with col_credit:
  ...
  with c_num:
    st.markdown(
        ...
        f" padding-top: 5px;'><b>{st.session_state.credits}</b></h2>",
        unsafe_allow_html=True,
    )
  with c_img:
...
st.markdown(
    '<div class="main-title">Assistant IA Personnel</div>', unsafe_allow_html=True
)
st.markdown(
    '<div class="sub-title">Propulsé par Google Gemini • Pose tes questions'
    " ci-dessous</div>",
    unsafe_allow_html=True,
)
...
if prompt := st.chat_input("Écris ton message ici..."):
  if st.session_state.credits <= 0:
    st.error(
        "⚠️ Vous avez épuisé vos crédits ! Revenez demain ou rechargez votre"
        " compte."
    )
  else:
